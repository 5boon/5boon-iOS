//
//  FindPasswordViewReactorSpec.swift
//  TodayMood
//
...
class FindPasswordViewReactorSpec: QuickSpec {
    override func spec() {
        ...
        describe("find 액션 체크") {
            context("find action이 호출되면") {
                it("find api가 호출되어야 한다.") {
                    Stubber.register(userService.findPassword) { _ in .just(UserFixture.kanz) }
                    reactor.action.onNext(.find("kanz11", "kanz11@5boon.com"))
                    expect(Stubber.executions(userService.findPassword).count) == 1
                }
            }
        }
        ...
        describe("find 액션 체크") {
            context("찾는 아이디가 존재하지 않을 경우") {
                it("failedText에 결과 없음 메세지가 셋팅되어야 한다.") {
                    Stubber.register(userService.findPassword) { _ in .error(StubError()) }
                    reactor.action.onNext(.find("kanz11", "kanz11@5boon.com"))
